...
function App () {
  ...
  function onParagraphFinish () {
    ...
    const durationSeconds =
      Math.abs(finishTime.getTime() - startTime.getTime()) / 1000
    const characters = paragraph.length - 1

    // standardised word is 5 letters long
    const wpm =
      Math.round((characters / (durationSeconds / 60) / 5) * 100) / 100
    ...
    typedStatusList.forEach((typedStatus, index) => {
      if (typedStatus === TypedStatus.INCORRECT)
  ...
  function onKeyPress (e: React.KeyboardEvent) {
    ...
    const isLastWord =
      currentWordIndex === paragraphLengths[paragraphLengthIndex] - 1
    ...
    <>
      <div className='relative'>
        <div className='flex flex-col items-center mb-30'>
          <div
            className={`${
              textInput === currentWord.substring(0, textInput.length)
        ...
        <div className='w-full fixed flex md:flex-row right-1/2 translate-x-1/2 justify-center bottom-[16%] space-x-4'>
          <button
            className='relative button-secondary group'
            onClick={() => {
              setParagraphLengthIndex(
                prev => (prev + 1) % paragraphLengths.length
              )
              handleFocus()
            }}
          >
          ...
          <button
            ...
            className='relative !px-10 button flex flex-row justify-center space-x-3 group'
          >
          ...
          <button
            className='relative button-secondary group'
            onClick={() => {
              setWordsRangeIndex(prev => (prev + 1) % wordsRanges.length)
              handleFocus()
            }}
          >
